...
router.post('/add', auth.authenticateToken, checkRole.checkRole,(req,res,next)=>{
    ...
    connection.query(query,[product.name,product.categoryId,product.description,product.price,],(err,results)=>{
        if(!err){
            return res.status(200).json({message: "Produto adcionado com sucesso!"});
    ...
    connection.query(query,[product.name,product.categoryId,product.description,product.price,product.id],(err,results)=>{
        if(!err){
            if(results.affectedRows ==0){
                return res.status(404).json({message:"Nao foi encontrado produto com esse ID!"});
            }
...
router.patch('/updateStatus', auth.authenticateToken, checkRole.checkRole,(req,res,next)=>{
    ...
    connection.query(query,[user.status,user.id],(err,results)=>{
        if(!err){
            ...
            return res.status(200).json({message:"Status do produto foi atualiado com sucesso"});
