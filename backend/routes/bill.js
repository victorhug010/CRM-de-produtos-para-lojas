...
router.get('/getBills', auth.authenticateToken, (req, res) => {
    var query = "select id, uuid, name, email, contactNumber, paymentMethod, total, productDetails from bill";
...
router.delete('/delete/:id', auth.authenticateToken,(req,res,next)=>{
    ...
    connection.query(query,[id],(err,results)=>{
        ...
        if(results.affectedRows ==0){
            return res.status(404).json({message:"Nao foi encontrada bill com esse ID!"});
        }
        return res.status(200).json({message:"bill deletado com sucesso"});
...
router.get('/getById/:id', auth.authenticateToken,(req,res,next)=>{
    const id= req.params.id;
    var query = "select productDetails from bill where id=?";
    connection.query(query,[id],(err,results)=>{
        if(!err){
            return res.status(200).json(results);
